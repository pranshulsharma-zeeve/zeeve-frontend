...
type OverviewResponse = { success: boolean; data?: ZkSyncRollupOverview };
type BlockchainDetailsResponse = { success: boolean; data?: ZkSyncBlockchainDetails };

const ZKSYNC_ROLLUP_TYPE = "zksync";
...
type ZkSyncNodeInfra = {
  cloud?: string;
  managed?: boolean;
  region?: { region?: string; regionName?: string };
};

type ZkSyncNodeDetail = {
  name?: string;
  type?: string;
  status?: NodeNetworkStates;
  createdAt?: string;
  infra?: ZkSyncNodeInfra;
  metadata?: Record<string, unknown>;
};

type ZkSyncNodeRpcAccess = {
  http?: boolean;
  ws?: boolean;
  apis?: string[];
};

type ZkSyncNodeRpcConfig = {
  endpoint?: string;
  rpcAccess?: ZkSyncNodeRpcAccess;
...
const mapNode = (node: ZkSyncRollupNode): ZkSyncRollupNodeSummary => {
  const host =
...
const deriveBlockHeightFromNode = (node?: ZkSyncRollupNode) => {
...
const deriveNetworkStatus = (nodes?: ZkSyncRollupNode[]): NodeNetworkStates => {
  if (!nodes?.length) return "requested";
  if (nodes.some((node) => normalizeStatus(node.status) === "failed")) return "failed";
  if (nodes.some((node) => normalizeStatus(node.status) === "provisioning")) return "provisioning";
  return "ready";
};
...
const findNodeByIdentifier = (nodes: ZkSyncRollupNode[] | undefined, nodeId?: string): ZkSyncRollupNode | undefined => {
...
const deriveInfraFromMetadata = (metadata?: Record<string, unknown>): ZkSyncNodeInfra | undefined => {
  if (!metadata) return undefined;
...
const deriveNodeDetail = (node?: ZkSyncRollupNode): ZkSyncNodeDetail | undefined => {
  if (!node) return undefined;
  const metadata = ensureRecord(node.metadata);
  return {
    name: node.name ?? (metadata?.name as string | undefined),
    type: node.node_type ?? (metadata?.type as string | undefined),
    status: normalizeStatus(node.status ?? (metadata?.status as string | undefined)),
...
const deriveRpcAccessFromNode = (node?: ZkSyncRollupNode) => {
  if (!node) return undefined;
  const metadata = ensureRecord(node.metadata);
...
const deriveRpcConfigFromNode = (node?: ZkSyncRollupNode): ZkSyncNodeRpcConfig | undefined => {
  if (!node) return undefined;
  const metadata = ensureRecord(node.metadata);
  ...
  const config: ZkSyncNodeRpcConfig = {
...
const deriveNodeCredentials = (metadata?: Record<string, unknown>): ZkSyncNodeCredentials | undefined => {
  if (!metadata) return undefined;
...
const buildQuery = (serviceId?: string) => {
  const params = new URLSearchParams({ type: ZKSYNC_ROLLUP_TYPE });
  if (serviceId) {
...
const findArtifact = (artifacts: ZkSyncBlockchainArtifact[] | undefined, matcher: (name: string) => boolean) => {
  if (!artifacts?.length) return undefined;
  return artifacts.find((artifact) => matcher((artifact.name ?? "").toLowerCase()));
};

const parseWalletArtifacts = (artifacts?: ZkSyncBlockchainArtifact[]): ParsedWalletArtifact[] => {
...
const useZkSyncValidiumService = () => {
  ...
  const useZkSyncNodeList = (
    serviceId?: string,
  ): RequestState<{
    rpc: ZkSyncRollupNodeSummary[];
    prover: ZkSyncRollupNodeSummary[];
    dac: ZkSyncRollupNodeSummary[];
    zkevm?: ZkSyncRollupNodeSummary;
  }> => {
  ...
  const useZkSyncNodeSummary = (
    serviceId?: string,
  ): RequestState<{
    rpcNode?: ZkSyncRollupNodeSummary;
  ...
  const useZkSyncBlockNumber = (serviceId?: string): RequestState<{ data: { blockHeight?: number } }> => {
    const base = useRollupOverview(serviceId);
    const request = useMemo(() => {
  ...
  const useZkSyncBlockchainConfig = (serviceId?: string): RequestState<{ data?: Record<string, any> }> => {
  ...
  const useZkSyncGenesisFile = (serviceId?: string): RequestState<{ data?: Record<string, any> | string }> => {
    const base = useRollupBlockchainDetails(serviceId);
    const request = useMemo(() => {
      const artifacts = base.request.data?.data?.artifacts ?? [];
  ...
  const useZkSyncNodeInfo = (serviceId?: string, nodeId?: string): RequestState<{ data?: ZkSyncNodeDetail }> => {
  ...
  const useZkSyncNodeRpcConfig = (
    serviceId?: string,
    nodeId?: string,
  ): RequestState<{ data?: ZkSyncNodeRpcConfig }> => {
    const overview = useRollupOverview(serviceId);
    const request = useMemo(() => {
      const node = findNodeByIdentifier(overview.request.data?.data?.nodes, nodeId);
  ...
  const useZkSyncNodeRpcCredentials = (
    serviceId?: string,
    nodeId?: string,
  ): RequestState<{ data?: ZkSyncNodeCredentials }> => {
    const overview = useRollupOverview(serviceId);
    const request = useMemo(() => {
      const node = findNodeByIdentifier(overview.request.data?.data?.nodes, nodeId);
      const metadata = ensureRecord(node?.metadata);
  ...
  return {
    ...
    node: {
      summary: useZkSyncNodeSummary,
      info: useZkSyncNodeInfo,
      list: useZkSyncNodeList,
      rpc: {
        delete: stub,
        cred: useZkSyncNodeRpcCredentials,
        config: useZkSyncNodeRpcConfig,
      },
      zkevm: {
        config: stub,
        delete: stub,
    ...
    supernet: {
      ...
      blockNumber: useZkSyncBlockNumber,
      blockchainConfig: useZkSyncBlockchainConfig,
      cloudValidation: stub,
      createSupernet: stub,
      deleteNetwork: stub,
      explorerEndpoints: stub,
      genesisFile: useZkSyncGenesisFile,
      infraDetails: stub,
      l1Info: stub,
      networkHealth: stub,
      smartContracts: stub,
      ...
      walletDetails: useZkSyncWalletDetails,
      walletList: useZkSyncWalletList,
      supernetList: stub,
      trialInfo: stub,
      appsList: stub,
      appsDetails: stub,
...
export default useZkSyncValidiumService;
