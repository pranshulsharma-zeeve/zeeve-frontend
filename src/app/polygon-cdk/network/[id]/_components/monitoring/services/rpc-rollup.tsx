...
export default function RpcRollupServices() {
  ...
  const entries = Object.entries(serviceData || {}).filter(
    ([key]) => key.replace(/^\/+/, "").toLowerCase() === "rpc" || key.replace(/^\/+/, "").toLowerCase() === "dac",
  );
  const visibleEntries = showAll ? entries : entries.slice(0, 4);

  return (
    <div className="flex size-full min-h-[350px] flex-col overflow-hidden rounded-3xl border border-[#E1E1E1] text-black">
      ...
      <div className="flex grow flex-col bg-[#F5F5F5] p-4">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-1">
          {visibleEntries.flatMap(([serviceName, metrics], nodeIndex) => {
            ...
            return (
              <div key={nodeIndex} className="min-w-0 p-1">
                ...
                <div className="mt-3 flex flex-col gap-2 text-sm text-black sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className="flex items-center gap-2 overflow-visible text-xs font-normal">
                    <span title={rpcItem?.url} className="max-w-[160px] truncate sm:max-w-[200px]">
