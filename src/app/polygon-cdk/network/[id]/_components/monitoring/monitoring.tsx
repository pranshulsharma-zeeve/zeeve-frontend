...
const MonitoringDashboard = () => {
  ...
  const { overview } = usePolygonCdkDashboard();
  ...
  const fallbackToken = user?.token ?? getCookie("token");
  const token = isDemoNetwork ? demoToken || fallbackToken : fallbackToken;
  // const token = "d7bca92c2353e3e9e5dbae88d44b1b49";
  const { request: dashboardRequest, url: dashboardUrlConfig } = useVisionService().mainPage.dashboard();
  ...
  const dashboardUrl = isDemoNetwork ? getPolygonCdkDemoVizionUrl(ROUTES.VISION.API.DASHBOARD) : dashboardUrlConfig;
  const serviceUrl = isDemoNetwork ? getPolygonCdkDemoVizionUrl(ROUTES.VISION.API.SERVICE) : serviceUrlConfig;
  const rpcsslUrl = isDemoNetwork ? getPolygonCdkDemoVizionUrl(ROUTES.VISION.API.SERVICE_RPC_SSL) : rpcsslUrlConfig;
  const monitorUrl = isDemoNetwork
    ? getPolygonCdkDemoVizionUrl(ROUTES.VISION.API.SECURITY_MONITOR_RPC)
    : monitorUrlConfig;
  const securityMonitoringCreatedAt = overview?.created_at ?? "";
  const selectedHost = useMemo(() => {
    if (isDemoNetwork) {
      return getPolygonCdkDemoHost();
    }
  ...
  // useEffect(() => {
  //   if (!isRefreshedManual) return;
  //   void fetchMonitoringData();
  // }, [fetchMonitoringData, isRefreshedManual]);
