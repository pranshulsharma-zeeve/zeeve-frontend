"use client";
import { Heading, useTabsContext, Z4Button } from "@zeeve-platform/ui";
...
const NodesOverview = () => {
  ...
  return (
    <div className="col-span-12 flex flex-col gap-6 rounded-lg bg-white shadow-sm xl:col-span-6 2xl:col-span-5">
      ...
      <div className="flex flex-row flex-wrap items-center gap-3 px-6 text-sm text-[#696969]">
        {[
          ...
          { label: "ZkEVM", value: nodeCount?.zkevm ?? 0 },
          { label: "Prover", value: nodeCount?.prover ?? 0 },
          { label: "DAC", value: nodeCount?.dac ?? 0 },
        ].map((item) => (
          <span key={item.label}>
