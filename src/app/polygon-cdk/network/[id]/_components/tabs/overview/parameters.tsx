...
import { Tooltip, tx } from "@zeeve-platform/ui";
...
const Parameters: React.FC<ParametersProps> = ({ colorScheme = "primary", chain }) => {
  ...
  const rpcUrl = useMemo(() => {
    const overviewRpcNode = normalized?.overview?.nodes?.find((node) =>
      (node.node_type ?? "").toLowerCase().includes("rpc"),
    );
    const overviewEndpoint =
      typeof overviewRpcNode?.endpoint_url === "string" ? overviewRpcNode.endpoint_url : undefined;
    const metadata = normalized?.rpcNode?.metadata;
    const metadataEndpoint = metadata && typeof metadata.endpoint === "string" ? metadata.endpoint : undefined;
    const fallback =
      normalized?.rpcNode?.rpcAccess?.http?.endpoint ??
      normalized?.rpcNode?.endpoint ??
      metadataEndpoint ??
      overviewEndpoint;
    return normalizeHttpUrl(fallback);
  }, [normalized]);
  ...
  return (
    <div className="col-span-12 flex h-full flex-col xl:col-span-4 2xl:col-span-2">
