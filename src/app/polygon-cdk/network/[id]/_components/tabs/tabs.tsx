...
const DashboardTabs = () => {
  const { isLoading, error, refresh } = usePolygonCdkDashboard();
  const searchParams = useSearchParams();
  const tabParam = searchParams.get("tab");

  // Map tab query param to tab index: monitoring -> 5 (0-indexed)
  const defaultTabIndex = tabParam === "monitoring" ? 5 : 0;

  if (isLoading) {
    return (
      <div className="rounded-lg border border-dashed border-brand-outline p-6 text-center text-sm text-brand-gray">
        Loading Polygon CDK dashboard...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-brand-red/50 p-6 text-center text-sm text-brand-dark">
        <p>Failed to load Polygon CDK data: {error}</p>
