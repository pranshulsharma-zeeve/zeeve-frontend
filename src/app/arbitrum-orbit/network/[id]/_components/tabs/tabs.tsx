...
const DashboardTabs = ({ defaultTabIndex }: { defaultTabIndex: number }) => {
  ...
  if (!networkInfo?.isLoading && !networkInfo?.data) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-brand-red/50 p-6 text-center text-sm text-brand-dark">
        <p>Failed to load Arbitrum data</p>
      </div>
    );
  }

  return (
    <Z4Tabs orientation="horizontal" className="rounded-none border-none" defaultIndex={defaultTabIndex}>
...
const IdPageTabs = () => {
  ...
  // Map tab query param to tab index: monitoring -> future tab index, default -> 0
  const initialTabIndex = tabParam === "monitoring" ? 4 : 0;
  const defaultTabIndex = initialTabIndex; // Currently no monitoring tab, defaulting to Overview
  const { id } = useParams();
  ...
  return <DashboardTabs defaultTabIndex={defaultTabIndex} />;
};
