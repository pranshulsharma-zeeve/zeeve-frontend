    ...
    <div className={`${className} col-span-12 flex flex-col rounded-2xl border border-[#89BBF54D] lg:flex-row`}>
      ...
      >
            ...
            <div className="flex flex-col gap-y-6 rounded-xl bg-[linear-gradient(180deg,rgba(108,0,246,0.2)_0%,rgba(108,0,246,0)_32.57%,rgba(108,0,246,0)_99.63%)] px-6 py-7 text-center">
              <div className="flex flex-col">
                <div className="text-4xl font-semibold text-white">
                  {dashboard?.data?.rollUpStatus?.totalVerifiedBatches ?? "NA"}
                </div>
                <div className="mt-1 text-sm font-normal text-[#AAABB8]">Total Verified Batches</div>
              </div>
              {/* <div className="flex flex-col">
                <div className="text-4xl font-semibold text-white">
        ...
        <div className=" flex flex-col rounded-lg p-3 text-center text-2xl font-semibold">
          <span className="text-2xl font-semibold"># {dashboard?.data?.rollUpStatus?.lastBatchSequenced}</span>
          <label className="mt-1 text-sm font-normal text-[#FFFFFFB2]">Last Batch Sequenced</label>
          <div className="mt-6">
            <Link
              as={"a"}
              target="_blank"
              rel="noopener noreferrer"
              href={networkInfo?.data?.rollup_metadata?.l3?.explorerUrl || "#"}
            >
              <Button
