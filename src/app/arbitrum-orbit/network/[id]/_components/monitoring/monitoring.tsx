...
import {
  getArbitrumDemoHost,
  getArbitrumDemoVizionToken,
  getArbitrumDemoVizionUrl,
  isArbitrumDemoNetwork,
} from "./demo-vizion";
...
const MonitoringDashboard = () => {
  ...
  const isDemoNetwork = isArbitrumDemoNetwork(networkId);
  const setDashboard = useArbitrumDashboardStore((state) => state.setArbitrumDashboard);
  ...
  const demoToken = getArbitrumDemoVizionToken();
  const token = isDemoNetwork ? demoToken : (user?.token ?? getCookie("token"));
  const { request: dashboardRequest, url: dashboardUrlConfig } = useVisionService().mainPage.arbitrumDashboard();
  ...
  const dashboardUrl = isDemoNetwork
    ? getArbitrumDemoVizionUrl(ROUTES.VISION.API.ARBITRUM_DASHBOARD)
    : dashboardUrlConfig;
  const serviceUrl = isDemoNetwork ? getArbitrumDemoVizionUrl(ROUTES.VISION.API.SERVICE) : serviceUrlConfig;
  const rpcsslUrl = isDemoNetwork ? getArbitrumDemoVizionUrl(ROUTES.VISION.API.SERVICE_RPC_SSL) : rpcsslUrlConfig;
  const monitorUrl = isDemoNetwork
    ? getArbitrumDemoVizionUrl(ROUTES.VISION.API.SECURITY_MONITOR_RPC)
    : monitorUrlConfig;
  const securityMonitoringCreatedAt = networkInfo?.data?.created_at ?? "";
  const selectedHost = useMemo(() => {
    if (isDemoNetwork) {
      return getArbitrumDemoHost();
    }
  ...
  const getPrimaryHost = useCallback((host: typeof selectedHost) => {
    if (!host) return "";
    if (Array.isArray(host.Sequencer) && host.Sequencer.length > 0) {
      return host.Sequencer[0];
    }
