...
const NitroNodeIdPageClient = () => {
  ...
  } = useArbitrumOrbitService().node.detail(networkId, nodeId);
  ...
  const networkPath = `${ROUTES.ARBITRUM_ORBIT.PAGE.NETWORK}/${networkId}`;
  const items = [
    {
    ...
    },
    {
      href:
        from === "workspace"
          ...
          : withBasePath(ROUTES.ARBITRUM_ORBIT.PAGE.LIST),
      label: from === "workspace" ? "Workspaces" : "Arbitrum Orbit",
    },
    {
  ...
  return (
    <div className="flex flex-col gap-y-2 lg:gap-y-6">
      <Navigation
        ...
        logo={
          <Image
            src={withBasePath("/assets/images/protocol/arbitrum-orbit.svg")}
            alt="arbitrum-orbit-logo"
            width={32}
            height={32}
          />
        }
