  ...
  return (
    <div
      className={`group col-span-4 mb-6 rounded-2xl border p-0 shadow-md transition-all lg:col-span-1 ${
        isSelected ? "border-brand-archive bg-brand-archive-10 text-white !shadow-none" : "bg-white"
      }`}
    >
        ...
        {PLAN_FIELDS.map(({ key, label }, index) => {
          ...
          if (typeof value === "boolean") {
            return (
              <div
                key={String(key)}
                className={`flex w-full items-center justify-between px-3 py-5 text-sm text-gray-700 ${bgColor} lg:justify-center`}
              >
        ...
        <div className={`w-full px-5 py-10`}>
          <Z4Button
            className={`w-full rounded-[4px] !py-6 text-sm font-semibold ${
              isSelected ? "bg-white text-brand-archive" : "bg-brand-archive text-white"
            }`}
            onClick={() => {
              if (isSelected && protocolId) {
                ...
                router.push(
                  `${ROUTES.PLATFORM.PAGE.MANAGE_ARCHIVE_NODES_PROTOCOLS}/purchase?${queryParams.toString()}`,
                );
              } else {
                onSelect();
              }
