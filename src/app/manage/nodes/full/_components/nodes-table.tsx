...
const NodesTable = (props: NodeTableProps) => {
  ...
  const nodeTableColumn: ColumnDef<NetworkListResponseData, any>[] = [
    ...
    //     return status ? <Status status="success" /> : <Status status="retry" />;
    //   },
    // }),
    ...
    // columnHelper.accessor("nodeSubscriptionRenewalDate", {
    ...
    return (
      <div className="flex flex-wrap items-center gap-2 lg:flex-nowrap lg:gap-3">
        {isDraft ? (
          ...
          <Tooltip
            text={status === "provisioning" ? `Node is in ${status}` : "Node Details are available"}
            placement={"top-start"}
          >
            <Link
              href={{
                pathname: `${ROUTES.PLATFORM.PAGE.MANAGE_FULL_NODES}/${nodeId}`,
                query: {
                  protocolId,
            ...
            >
              <Button
                ...
                onClick={() => {
                  localStorage.setItem("from", "fullnode");
                }}
              >
  ...
  return (
    <div className="flex flex-col gap-3 overflow-hidden lg:gap-6">
      <div className="flex flex-col items-center justify-between gap-2 lg:flex-row">
        <Heading as="h4">RPC Nodes ({filteredData?.length ?? "Loading..."}) </Heading>
      ...
      <ReactTable
        ...
        pagination={{
          ...
          pageCount: filteredData?.length && pageSize ? Math.ceil((filteredData?.length ?? 0) / pageSize) : 0,
          pageIndex,
          pageSize,
          setPagination,
        }}
