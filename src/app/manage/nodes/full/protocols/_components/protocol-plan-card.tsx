  ...
  return (
    <div
      className={`group col-span-4 mb-6 rounded-2xl border p-0 shadow-md transition-all lg:col-span-1 ${
        isSelected ? "border-brand-full bg-brand-full-10  text-white" : "bg-white"
      }`}
    >
        ...
        <div className={`w-full px-5 py-10 `}>
          <Z4Button
            className={`w-full rounded-[4px] !py-6 text-sm font-semibold ${
              isSelected ? "bg-white text-brand-full" : "bg-brand-full text-white"
            }`}
            onClick={() => {
              if (isSelected && protocolId) {
                ...
                router.push(`${ROUTES.PLATFORM.PAGE.MANAGE_FULL_NODES_PROTOCOLS}/purchase?${queryParams.toString()}`);
              } else {
                onSelect();
              }
            }}
