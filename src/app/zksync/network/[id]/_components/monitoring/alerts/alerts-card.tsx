...
const AlertsCard = ({
  ...
  const alertsUrl = isDemoNetwork ? getZkSyncDemoVizionUrl(ROUTES.VISION.API.ALERTS) : alertsUrlConfig;
  ...
  const token = isDemoNetwork ? getZkSyncDemoVizionToken() : (user?.token ?? getCookie("token"));
  ...
  const fetchAlertsData = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === HTTP_STATUS.OK && Array.isArray(data.data.data)) {
        const filteredAlerts = filterAlertsWithinDays(data.data.data);
