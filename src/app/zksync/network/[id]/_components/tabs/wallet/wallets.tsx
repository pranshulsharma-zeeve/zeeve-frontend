  ...
  return (
    <div className="flex gap-4 lg:flex-row">
      {typeof row.original.explorerUrl === "string" ? (
        ...
        <>
          <Tooltip text={"View on L1 explorer"} placement={"top-start"}>
            <IconButton
              colorScheme={"primary"}
              variant={"ghost"}
...
const getBalanceColumns = (isBalanceLoading: boolean = false) => [
  columnHelper.accessor("balanceL1", {
    ...
    cell: (info) => {
      ...
      return (
        <Tooltip text={`${info.row.original.balanceL1} ETH`} placement="top-start">
          <div>{convertNumber(info.row.original.balanceL1)} ETH</div>
        </Tooltip>
      );
    },
  }),
  columnHelper.accessor("balanceL2", {
    ...
    cell: (info) => {
      ...
      if (isEmpty) return "Not Enough Data";
      return (
        <Tooltip text={`${info.row.original.balanceL2} ETH`} placement="top-start">
          <div>{convertNumber(info.row.original.balanceL2)} ETH</div>
        </Tooltip>
      );
    },
  }),
