...
import type { WalletListItem } from "@/services/zksync-validium/supernet/wallet-list";
import { useZksyncStore } from "@/store/zksync";

type ZkSyncRollupLayerMetadata = {
  name?: string;
...
type ZkSyncRollupMetadata = {
  l1?: ZkSyncRollupLayerMetadata;
  l2?: ZkSyncRollupLayerMetadata;
  l3?: ZkSyncRollupLayerMetadata;
...
type ZkSyncOverviewData = ZkSyncRollupOverview & {
  status?: string;
  chain_id?: string | number;
  rollup_metadata?: ZkSyncRollupMetadata;
  created_at?: string;
};

type ZkSyncSmartContract = {
  name?: string;
  address?: string;
  explorer?: string;
};

type ZkSyncBlockchainDetailsData = ZkSyncBlockchainDetails & {
  status?: string;
  chain_id?: string | number;
  smartContracts?: { l1?: ZkSyncSmartContract[]; l2?: ZkSyncSmartContract[] };
  smart_contracts?: { l1?: ZkSyncSmartContract[]; l2?: ZkSyncSmartContract[] };
...
type ZkSyncDashboardData = {
  overview: ZkSyncOverviewData | null;
  blockchainDetails: ZkSyncBlockchainDetailsData | null;
  summary?: ZkSyncSummary;
  supernetSummary?: SupernetSummary;
  nodes: NormalizedNode[];
  nodesByType: Partial<Record<NodeType, NormalizedNode[]>>;
  nodeCount: ZkSyncRollupNodeCount;
  rpcNode?: NormalizedNode;
  userInputs: ZkSyncRollupUserInputs;
  rollupMetadata?: ZkSyncRollupMetadata;
  blockHeight?: number;
  ...
  smartContracts: {
    l1?: ZkSyncSmartContract[];
    l2?: ZkSyncSmartContract[];
    explorerL1?: string;
    explorerL2?: string;
  };
...
type ZkSyncDashboardState = {
  overview: ZkSyncOverviewData | null;
  blockchainDetails: ZkSyncBlockchainDetailsData | null;
  normalized?: ZkSyncDashboardData;
  isLoading: boolean;
  error?: string;
  lastUpdated?: number;
};

type ZkSyncDashboardContextValue = ZkSyncDashboardState & {
  refresh: () => Promise<void>;
};

const ZkSyncDashboardContext = createContext<ZkSyncDashboardContextValue | undefined>(undefined);
...
const calculateNodeCount = (nodes?: ZkSyncRollupNode[]): ZkSyncRollupNodeCount => {
  if (!nodes?.length) return { total: 0 };
  return nodes.reduce<ZkSyncRollupNodeCount>(
    (acc, node) => {
...
const mapNode = (node: ZkSyncRollupNode): NormalizedNode => {
...
const findArtifact = (
  artifacts: ZkSyncBlockchainArtifact[] | undefined,
  matcher: (name: string) => boolean,
): ZkSyncBlockchainArtifact | undefined => {
  if (!artifacts?.length) return undefined;
  return artifacts.find((artifact) => matcher((artifact.name ?? "").toLowerCase()));
};

const parseWalletArtifacts = (artifacts?: ZkSyncBlockchainArtifact[]): WalletCollections => {
  ...
  const pushEntry = (record: Record<string, unknown>, fallbackName?: string) => {
    ...
    const type =
      (typeof record.type === "string" && record.type.trim() && record.type) ||
    ...
    const source = sourceValue?.toLowerCase().includes("l2") ? "l2" : "l1";
    if (source === "l2") collections.l2.push(entry);
    else collections.l1.push(entry);
  };
...
const collectContractsFromValue = (value: unknown, fallbackLabel?: string): ZkSyncSmartContract[] => {
  const contracts: ZkSyncSmartContract[] = [];
...
const parseContractArtifact = (artifactContent?: Record<string, unknown>) => {
  ...
  const l1Contracts: ZkSyncSmartContract[] = [];
  const l2Contracts: ZkSyncSmartContract[] = [];
...
const normalizeContractList = (list?: ZkSyncSmartContract[]) => {
  if (!list?.length) return undefined;
  const normalized = list
    .filter((contract): contract is ZkSyncSmartContract => Boolean(contract?.address && isHexAddress(contract.address)))
    .map((contract) => ({
...
const mergeSmartContractLists = (
  ...lists: Array<ZkSyncSmartContract[] | undefined>
): ZkSyncSmartContract[] | undefined => {
  const seen = new Map<string, ZkSyncSmartContract>();
  lists.forEach((list) => {
...
const deriveBridgeInfo = (metadata?: ZkSyncRollupMetadata) => {
...
const extractGenesisFile = (blockchainDetails: ZkSyncBlockchainDetailsData | null) => {
...
const deriveSummary = (
  overview: ZkSyncOverviewData | null,
  userInputs: ZkSyncRollupUserInputs,
  metadata?: ZkSyncRollupMetadata,
): { summary?: ZkSyncSummary; supernetSummary?: SupernetSummary } => {
...
): { l1?: ZkSyncSmartContract[]; l2?: ZkSyncSmartContract[] } => {
...
const extractBlockchainConfig = (
  rollupMetadata: ZkSyncRollupMetadata | undefined,
  blockchainDetails: ZkSyncBlockchainDetailsData | null,
) => {
...
const normalizeDashboard = (
  ...
  const overviewGeneralConfig = overview?.generalConfig;
  const rollupMetadataBase = overview?.rollup_metadata;
  const mergedGeneralConfig = overviewGeneralConfig
    ? (() => {
        const baseConfig = (rollupMetadataBase?.generalConfig ?? {}) as Partial<GENERALCONFIG> &
          Record<string, unknown>;
        const baseDaClient = baseConfig.daClient;
        const incomingDaClient = overviewGeneralConfig.daClient;
        let mergedDaClient: typeof baseDaClient = baseDaClient;
        if (typeof incomingDaClient === "string") {
          mergedDaClient = incomingDaClient;
        } else if (incomingDaClient && typeof incomingDaClient === "object") {
          if (baseDaClient && typeof baseDaClient === "object") {
            mergedDaClient = { ...baseDaClient, ...incomingDaClient };
          } else {
            mergedDaClient = incomingDaClient;
          }
        }
        return {
          ...baseConfig,
          ...overviewGeneralConfig,
          rpcConfig: {
            ...(baseConfig.rpcConfig ?? {}),
            ...(overviewGeneralConfig.rpcConfig ?? {}),
          },
          transactionMempool: {
            ...(baseConfig.transactionMempool ?? {}),
            ...(overviewGeneralConfig.transactionMempool ?? {}),
          },
          daClient: mergedDaClient,
        };
      })()
    : rollupMetadataBase?.generalConfig;
  const rollupMetadata = rollupMetadataBase
    ? mergedGeneralConfig
      ? { ...rollupMetadataBase, generalConfig: mergedGeneralConfig }
      : rollupMetadataBase
    : mergedGeneralConfig
      ? { generalConfig: mergedGeneralConfig }
      : undefined;
  const blockHeight = firstDefined(
    rollupMetadata?.l2?.blockHeight ? toNumber(rollupMetadata.l2.blockHeight) : undefined,
    rollupMetadata?.l3?.blockHeight ? toNumber(rollupMetadata.l3.blockHeight) : undefined,
    ...nodes.map((node) => deriveBlockHeightFromNode(node)),
  );
  const userInputs = (overview?.user_inputs as ZkSyncRollupUserInputs) ?? {};
...
const ZkSyncDashboardProvider = ({ serviceId, children }: ProviderProps) => {
  const [lastUpdated, setLastUpdated] = useState<number | undefined>(undefined);
  const setSuperNetInfo = useZksyncStore((state) => state.setZksyncInfo);

  const { supernet } = useZkSyncValidiumService();
  ...
  const overviewData = (overviewRequest.data?.data as ZkSyncOverviewData | undefined) ?? null;
  const blockchainDetailsData =
    (blockchainDetailsRequest.data?.data as ZkSyncBlockchainDetailsData | undefined) ?? null;
  ...
  const value = useMemo<ZkSyncDashboardContextValue>(
    () => ({
  ...
  return <ZkSyncDashboardContext.Provider value={value}>{children}</ZkSyncDashboardContext.Provider>;
};

const useZkSyncDashboard = () => {
  const context = useContext(ZkSyncDashboardContext);
  if (!context) {
    throw new Error("useZkSyncDashboard must be used within ZkSyncDashboardProvider");
  }
  return context;
};
...
export type {
  ZkSyncDashboardData,
  ZkSyncOverviewData,
  ZkSyncBlockchainDetailsData,
  ZkSyncSmartContract,
  WalletCollections,
  WalletHighlight,
};
export { ZkSyncDashboardProvider, useZkSyncDashboard };
