...
import { normalizeStatus } from "../page-client";
...
const DeployedNetworkCard = ({ id, name, status, environment, inputs }: Props) => {
  ...
  const useLightTheme = isMainnet;
  const labelText = isMainnet ? "MAINNET" : "TESTNET";
  const detailsUrl = `/zksync/network/${id}`;
  ...
  const cardType = useLightTheme ? ("testnet" as const) : isMainnet ? ("mainnet" as const) : ("testnet" as const);
  const buttonScheme = useLightTheme ? ("testnet" as const) : isMainnet ? ("mainnet" as const) : ("testnet" as const);
  ...
  const handleCompletePayment = async () => {
    ...
    try {
      ...
      if (serviceId) {
        router.push(`/zksync/network/${serviceId}`);
        return;
      }
  ...
  return (
    <div className="col-span-12 flex size-full flex-col lg:col-span-4">
      <Z4DashboardCard cardType={cardType} className="flex size-full grow flex-col">
        <div className={tx("flex flex-col items-start gap-3 text-sm")}>
          <Heading
            className={tx(
              {
                "text-white": isMainnet && !useLightTheme,
                "text-brand-testnet": !isMainnet,
                "text-[#09122D]": useLightTheme,
              },
          ...
          {isMainnet
            ? "Layer 2 or Layer3 zk Rollup Mainnet using zkSync ZK Stack."
            : "Layer 2 zk Rollup Testnet using zkSync ZK Stack."}
        </div>
        <KeyValuePair
          label="Name"
          ...
          {isDraft ? (
            <>
              ...
              <Link className="w-full" href={docsUrl} target="_blank">
                <Z4Button className="h-12 w-full" colorScheme={buttonScheme} variant="outline">
                  Info
                </Z4Button>
              </Link>
            </>
            ...
            <>
              <Link className="w-full" href={detailsUrl}>
                <Z4Button className="h-12 w-full" colorScheme={buttonScheme} variant="solid">
                  View
                </Z4Button>
              </Link>
              <Link className="w-full" href={docsUrl} target="_blank">
