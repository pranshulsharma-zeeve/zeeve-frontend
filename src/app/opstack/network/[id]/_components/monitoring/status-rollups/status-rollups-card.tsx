...
import { useOpStackDashboardStore } from "@/store/vizion/opstack-dashboard";
import {
  getL2ExplorerUrl,
  getL3ExplorerUrl,
  getSettlementLayer,
} from "@/modules/arbitrum-orbit/utils/network-overview";

const StatusRollupsCard = ({ className }: { className?: string }) => {
  const dashboard = useOpStackDashboardStore((state) => state.opStackDashboard);
  const networkInfo = useNetworkStore((state) => state.networkInfo);
  const settlementLayer = getSettlementLayer(networkInfo?.data);

  // Get explorer URL dynamically based on settlement layer
  const getExplorerUrl = () => {
    if (!settlementLayer) return getL2ExplorerUrl(networkInfo?.data);
    ...
    if (match) {
      ...
      return layerNumber === 3 ? getL3ExplorerUrl(networkInfo?.data) : getL2ExplorerUrl(networkInfo?.data);
    }
    return getL2ExplorerUrl(networkInfo?.data);
  };

  return (
    <div className={`${className} col-span-12 flex flex-col rounded-2xl border border-[#89BBF54D] lg:flex-row`}>
      <Card
        className={`w-full rounded-l-xl rounded-r-none border border-[#1c2b62] p-4 text-xl font-medium text-white lg:w-[45%] lg:px-5`}
        style={{
      ...
      >
        <div className="relative mt-8 flex justify-center">
          ...
          <div className="w-full rounded-xl bg-gradient-to-b from-[rgba(108,0,246,0.4)] via-[rgba(108,0,246,0.15)] to-transparent p-2">
            ...
            <div className="flex flex-col gap-y-6 rounded-xl bg-[linear-gradient(180deg,rgba(108,0,246,0.2)_0%,rgba(108,0,246,0)_32.57%,rgba(108,0,246,0)_99.63%)] px-6 py-7 text-center">
              <div className="flex flex-col">
                <div className="text-4xl font-semibold text-white">
                  {dashboard?.data?.rollUpStatus?.totalConfirmedBatches ?? "NA"}
                </div>
                <div className="mt-1 text-sm font-normal text-[#AAABB8]">Total Confirmed Batches</div>
              </div>
              {/* <div className="flex flex-col">
                <div className="text-4xl font-semibold text-white">
                  {dashboard?.data?.rollUpStatus?.totalSequencedBatches ?? "NA"}
                </div>
                <div className="mt-1 text-sm font-normal text-[#AAABB8]">Total Sequenced Batches</div>
              </div> */}
            </div>
            ...
            {/* <div className="mt-8 grid grid-cols-2 gap-4 text-left">
              <div className="flex min-w-0 flex-col break-words">
                ...
                <span className="text-xs font-normal">
                  {dashboard?.data?.rollUpStatus?.timeSinceLastSequencedBatch || "NA"}
                </span>
              </div>
        ...
        <div className=" flex flex-col rounded-lg p-3 text-center text-2xl font-semibold">
          <span className="text-4xl font-semibold text-white">
            {dashboard?.data?.rollUpStatus?.totalPublishedBatches ?? "NA"}
          </span>
          <label className="mt-1 text-sm font-normal text-[#FFFFFFB2]">Total Published Batches</label>
          <div className="mt-6">
            <Link as={"a"} target="_blank" rel="noopener noreferrer" href={getExplorerUrl() ?? "#"}>
              <Button
                isDisabled={networkInfo?.isLoading}
        ...
        <div className="flex flex-col gap-4 pt-16 md:flex-row md:justify-between">
          <div className="flex items-center justify-between text-sm font-normal text-[#AAABB8]">
            <div className="flex items-center gap-2">
              ...
              <div className="">
                ...
                <span className="block text-xs font-medium text-[#0BB63B]">
                  {dashboard?.data?.rollUpStatus?.proofs && dashboard.data.rollUpStatus.proofs.trim()
                    ? dashboard.data.rollUpStatus.proofs
                    : "Fault Proofs"}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between text-sm font-normal text-[#AAABB8]">
            <div className="flex items-center gap-2">
              ...
              <div className="">
                ...
                <span className="block text-xs font-medium text-[#0BB63B]">
                  {dashboard?.data?.rollUpStatus?.protocolVersion && dashboard.data.rollUpStatus.protocolVersion.trim()
                    ? dashboard.data.rollUpStatus.protocolVersion
                    : "op-node:v3.6.8"}
                </span>
              </div>
            </div>
