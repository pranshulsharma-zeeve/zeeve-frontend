...
const MonitoringDashboard = () => {
  ...
  const setDashboard = useOpStackDashboardStore((state) => state.setOpStackDashboard);
  ...
  const demoToken = getOpStackDemoVizionToken();
  const token = isDemoNetwork ? demoToken : (user?.token ?? getCookie("token"));
  const { request: dashboardRequest, url: dashboardUrlConfig } = useVisionService().mainPage.opStackDashboard();
  ...
  const dashboardUrl = isDemoNetwork
    ? getOpStackDemoVizionUrl(ROUTES.VISION.API.OPSTACK_DASHBOARD)
    : dashboardUrlConfig;
  const serviceUrl = isDemoNetwork ? getOpStackDemoVizionUrl(ROUTES.VISION.API.SERVICE) : serviceUrlConfig;
  const rpcsslUrl = isDemoNetwork ? getOpStackDemoVizionUrl(ROUTES.VISION.API.SERVICE_RPC_SSL) : rpcsslUrlConfig;
  const monitorUrl = isDemoNetwork ? getOpStackDemoVizionUrl(ROUTES.VISION.API.SECURITY_MONITOR_RPC) : monitorUrlConfig;
  ...
  const fetchMonitoringData = useCallback(async () => {
    ...
    // const hostIds = ["10800", "10797", "10799", "10798", "10796"];
    const primaryHost = getPrimaryHost(selectedHost);
    // const primaryHost = "10796";
