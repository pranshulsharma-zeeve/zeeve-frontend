...
const FinancialsTransactionsCard = ({ className }: { className?: string }) => {
  ...
  const formatDecimals = (value?: string | number) => {
    ...
    return Number.isFinite(numericValue) ? numericValue.toFixed(9) : "NA";
  };
      ...
      <div className="relative mb-2">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <KeyValue
            ...
            label="Batcher Balance"
            eth="redEth"
            value={formatDecimals(dashboard?.data.financials.batcherBalance) || "0"}
            valueClassName="border border-[#DE4841] rounded-md p-1 text-[#DE4841]"
          />
          <KeyValue
            label="L1 Base Fee Used for Batch Posting"
            eth="gwei"
            labelClassName="text-[#696969]"
            valueClassName="border border-[#DC3090] rounded-md p-1 text-[#DC3090]"
            value={
              dashboard?.data.financials.baseFee ? formatDecimals(Number(dashboard.data.financials.baseFee) / 1e9) : "0"
            }
          />
          <KeyValue
            labelClassName="text-[#696969]"
            label="Blob Base Fee Used for Batch Posting"
            eth="gwei"
            value={
              dashboard?.data.financials.blobBaseFee
                ? formatDecimals(Number(dashboard.data.financials.blobBaseFee) / 1e9)
                : "0"
            }
            valueClassName="border border-[#E59D44] rounded-md p-1 text-[#E59D44]"
          />
          <KeyValue
            label="Total Batcher Fees (since reset)"
            eth="gwei"
            labelClassName="text-[#696969]"
            valueClassName="border border-[#5133D3] rounded-md p-1 text-[#5133D3]"
            value={
              dashboard?.data.financials.totalBatcherFees
                ? formatNumber(Number(dashboard.data.financials.totalBatcherFees), "standard")
                : "0"
            }
          />
        </div>
      ...
      {listGraph ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <KeyValue
              labelClassName="text-[#696969]"
              label="Proposer Balance"
              eth="greenEth"
              value={formatDecimals(dashboard?.data.financials.proposerBalance) || "0"}
              valueClassName="border border-[#0BB63B] rounded-md p-1 text-[#0BB63B]"
            />
          </div>
